...
@section('content')
    <div class="space-y-6">
        ...
        {{-- HEADER CARD (Matching Learner Dashboard) --}}
        <x-ui.card class="bg-gradient-to-r from-ds-navy to-[#0F4C81] text-white border-none overflow-hidden relative">
            ...
            <div class="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
                ...
                <div class="flex items-center gap-3">
                    <x-ui.button variant="primary" href="{{ route('partner.learners.create') }}"
                        class="shadow-lg shadow-blue-900/20 border-white/10">
                        <x-slot name="icon">
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-slate-50/50 border-b border-slate-200">
                            <th class="px-6 py-4 font-bold text-slate-600 text-xs uppercase tracking-wider">Learner Details
                            </th>
                            <th class="px-6 py-4 font-bold text-slate-600 text-xs uppercase tracking-wider">Account Status
                            </th>
                            <th class="px-6 py-4 font-bold text-slate-600 text-xs uppercase tracking-wider">College Approval
                            </th>
                            <th class="px-6 py-4 font-bold text-slate-600 text-xs uppercase tracking-wider">Microsoft 365
                            </th>
                            <th class="px-6 py-4 text-right font-bold text-slate-600 text-xs uppercase tracking-wider">
                                Actions</th>
                        </tr>
                    </thead>
                        ...
                        @forelse($learners as $learner)
                            <tr class="group hover:bg-slate-50/80 transition-colors">
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        <div
                                            class="h-10 w-10 shrink-0 rounded-full bg-slate-100 flex items-center justify-center text-slate-600 font-bold text-sm border border-slate-200 shadow-sm">
                                            {{ substr($learner->first_name, 0, 1) }}{{ substr($learner->sur_name, 0, 1) }}
                                        </div>
                                        <div>
                                            <div class="font-bold text-slate-900 text-sm">{{ $learner->first_name }}
                                                {{ $learner->sur_name }}</div>
                                ...
                                <td class="px-6 py-4">
                                    @if($learner->ms_user_id)
                                        <div class="flex items-center gap-1.5 text-slate-700">
                                            <svg class="h-4 w-4 text-ds-pink" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                                stroke-width="2">
                                                <path d="M1 1h10v10H1V1zM13 1h10v10H13V1zM1 13h10v10H1V13zM13 13h10v10H13V13z" />
                                            </svg>
                                            <span class="text-xs font-semibold">Provisioned</span>
                                        </div>
                                    @else
                                ...
                                <td class="px-6 py-4 text-right">
                                    <x-ui.button variant="ghost" size="sm"
                                        href="{{ route('partner.learners.show', $learner->id) }}">
                                        Manage
                                    </x-ui.button>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="5" class="px-6 py-16 text-center">
                                    ...
                                    <p class="mt-1 text-xs text-slate-500">Get started by creating a new learner account.</p>
                                    <div class="mt-4">
