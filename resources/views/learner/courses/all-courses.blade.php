    ...
    <div class="max-w-7xl mx-auto">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
            <div>
             ...
             @if ($enrolments->count())
                <div class="inline-flex items-center gap-2 px-4 py-2 bg-white border border-slate-200 rounded-full shadow-sm text-sm font-medium text-slate-700">
                    <span class="flex items-center justify-center w-6 h-6 rounded-full bg-blue-100 text-blue-600 text-xs">📚</span>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                @foreach ($enrolments as $enrolment)
                    @php
                        ...
                        // If no order exists, treat active/paid/approved as paid
                        ...
                        // Gate: Verified AND (Active/Paid/Approved OR Paid by order)
                        $isActiveOrPaid = $isPaid || in_array($statusStr, ['active', 'paid', 'approved']);
                        $showContinue   = $isVerified && $isActiveOrPaid;

                        // Visuals
                        $isPaymentPending = $statusStr === 'pending-payment' || ($latestOrder && !$isOrderPaid);
                    ...
                    <x-ui.card class="h-full flex flex-col hover:shadow-md transition-shadow duration-200" padding="p-0">
                        <div class="p-5 flex-1 flex flex-col">
                            ...
                            <div class="flex items-start justify-between gap-2 mb-3">
                                @if ($course?->category)
                                    <span class="inline-flex px-2 py-1 rounded bg-slate-100 text-[10px] font-bold uppercase tracking-wider text-slate-600">
                            ...
                            <h3 class="font-bold text-lg text-slate-900 mb-2 line-clamp-2 leading-tight">
                             ...
                             <div class="mt-auto pt-4 flex items-center justify-between text-xs text-slate-400 border-t border-slate-50">
                        ...
                        <div class="p-4 bg-slate-50 border-t border-slate-100 flex items-center justify-between gap-3">
                             @if ($showContinue && $course)
                                <a href="{{ route('portal.learner.course.show', $enrolment->id) }}" class="w-full">
                                    <x-ui.button fullWidth size="sm">
                                        Continue Learning
                                        <x-slot name="icon"><svg class="w-4 h-4 ml-1" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14 5l7 7m0 0l-7 7m7-7H3" /></svg></x-slot>
                                    </x-ui.button>
                                </a>
                            @elseif ($isPaymentPending)
                                <span class="text-xs font-semibold text-amber-600 w-full text-center">Contact Partner for Payment</span>
                            ...
                            @elseif ($isPaidPending && !$isVerified)
                                 <span class="text-xs font-semibold text-slate-500 italic w-full text-center">Awaiting Admin Approval</span>
                            @elseif ($denied)
